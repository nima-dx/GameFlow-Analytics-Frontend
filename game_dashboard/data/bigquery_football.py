...
def get_football_kpis(league: str, season: str) -> dict:
    ...
    goals = int(filtered["score"].sum())
    xg = round(filtered["expected_goals"].sum(), 2)
    wins = int((filtered["points"] == 3).sum())
...
def get_team_form_data(league: str, season: str, team: str | None = None) -> pd.DataFrame:
    ...
    if filtered.empty:
        return pd.DataFrame(columns=["matchday", "points", "team"])
    ...
    team_df["points"] = team_df["points"].cumsum()

    return team_df[["matchday", "points", "team_name"]].rename(columns={"team_name": "team"})
...
def get_player_stats(league: str, season: str) -> pd.DataFrame:
    ...
    if filtered.empty:
        return pd.DataFrame(columns=["player", "team", "goals", "assists", "appearances"])

    filtered["goal_contribution"] = filtered["normal_goals"].fillna(0) + filtered["penalties"].fillna(0)

    filtered["assist_flag"] = filtered["assist_name"].fillna("").str.strip().ne("").astype(int)
...
def get_home_away_summary(league: str, season: str) -> pd.DataFrame:
    ...
    if filtered.empty:
        return pd.DataFrame(columns=[
            "team_name", "homeaway", "games", "wins", "win_rate", "avg_goals"
        ])
